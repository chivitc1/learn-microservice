...
public class RabbitMQConfiguration implements RabbitListenerConfigurer
{
	...
	public Queue gamificationMultiplicationQueue(
			@Value("${multiplication.queue}") final String _queueName
	)
	{
		return new Queue(_queueName, true);
	}

	@Bean
	public Binding binding(final Queue _queue, final TopicExchange _exchange,
		   @Value("${multiplication.anything.routing-key}") final String _routingKey) {
		return BindingBuilder.bind(_queue).to(_exchange).with(_routingKey);
	}
	 ...
	 * @param _registrar the registrar to be configured
	 */
	@Override
	public void configureRabbitListeners(RabbitListenerEndpointRegistrar _registrar)
	{
		_registrar.setMessageHandlerMethodFactory(messageHandlerMethodFactory());
	}
}
